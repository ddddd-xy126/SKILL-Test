<template>
  <div class="jsmpeg-player" @mouseenter="handlePlayerMouseEnter" @mouseleave="handlePlayerMouseLeave">
    <div class="player-header" :class="{ 'is-show': showTitle }">
      ...
      <div class="recording-tips" v-if="playerStatus.recording">
        <div class="recording-icon" />
        REC <template v-if="showTitle"> {{ recordingDurationLabel }} </template>
      </div>
      <button v-if="showCloseBtn" class="close-btn jm-icon-close" type="button" title="关闭"
        @click="$emit('close')"></button>
    </div>
    <div class="player-main" ref="player-main" @mousemove.passive="handleCanvasMouseMove" @click="handleCanvasClick"
      @dblclick="toggleFullscreen">
      <div class="player-loading-mask" v-show="loading">
        <div class="player-loading">
          <slot v-if="hasLoadingSlot" name="loading" :text="loadingText"></slot>
          <loading v-else :text="loadingText"></loading>
        </div>
      </div>
    ...
    <div class="player-toolbar" v-if="withToolbar" :class="{ 'is-show': playerStatus.playerHover }"
      @mouseenter="handleToolbarMouseEnter" @mouseleave="handleToolbarMouseLeave">
      <button class="toolbar-item play-btn" type="button"
        :class="paused ? 'jm-icon-video-play is-paused' : 'jm-icon-video-pause'" :title="paused ? '播放' : '暂停'"
        @click="handleToolbar('play')"></button>
      <button class="toolbar-item stop-btn jm-icon-stop" title="停止" type="button"
        @click="handleToolbar('stop')"></button>
      <button class="toolbar-item volume-btn" type="button" title="音量" v-popover:popover-volume
        :class="isMuted ? 'jm-icon-muted' : 'jm-icon-volume'" @click="handleToolbar('mute')"></button>
      ...
      <button class="toolbar-item snapshot-btn jm-icon-screenshots" title="截图" type="button"
        @click="handleToolbar('snapshot')"></button>
      <button class="toolbar-item recording-btn jm-icon-recording" type="button"
        :class="playerStatus.recording ? 'is-recording' : ''" :title="playerStatus.recording ? '停止录制' : '录制'"
        @click="handleToolbar('recording')"></button>
      <button class="toolbar-item setting-btn jm-icon-settings" title="设置" type="button"
        v-popover:popover-setting></button>
      <button class="toolbar-item fullscreen-btn" type="button" :class="playerStatus.fullscreen
        ? 'jm-icon-fullscreen-exit'
        : 'jm-icon-fullscreen'
        " :title="playerStatus.fullscreen ? '取消全屏' : '全屏'" @click="handleToolbar('fullscreen')"></button>
    </div>
    <div class="overlayers">
      <template v-if="withToolbar">
        <el-popover popper-class="jsmpeg-player-popover popover-setting" ref="popover-setting" trigger="hover"
          placement="top-end" :visible-arrow="popoverVisibleArrow" :append-to-body="false">
          ...
          <div class="setting-item" highlight>
            ...
            <div class="input__wrap">
              <el-switch class="input" v-model="playerSettings.autoStretch"
          ...
          <div class="setting-item" highlight>
            ...
            <div class="input__wrap">
              <button class="toolbar-item jm-icon-rotate-left" title="向左旋转90度" type="button"
                @click="rotate(-90, true)"></button>
              <button class="toolbar-item jm-icon-rotate-right" title="向右旋转90度" type="button"
                @click="rotate(90, true)"></button>
            </div>
          </div>
          <!-- <div class="setting-item">
        ...
        <el-popover popper-class="jsmpeg-player-popover popover-volume" ref="popover-volume" trigger="hover"
          placement="top" :visible-arrow="popoverVisibleArrow" :append-to-body="false">
          <div class="volume-value">{{ volumePercent }}</div>
          <el-slider v-model="volume" vertical height="120px" :max="1" :min="0" :step="0.01" :show-tooltip="false"
            :marks="{
              0: '',
              0.5: '',
              1: ''
