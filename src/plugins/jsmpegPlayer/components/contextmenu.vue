<template>
  <div ref="reference">
    ...
    <transition :name="transition" @after-enter="$emit('opened')" @after-leave="$emit('closed')">
      <div class="cl-contextmenu" ref="contextmenu" v-show="visible" :style="bindStyle" @click.stop @mousedown.stop
        @contextmenu.prevent>
        <template v-if="$slots['menu-item']">
          <ul class="menu-list">
            <li class="menu-item" v-for="(item, index) of menus" :key="index">
            </li>
          </ul>
        </template>
        <template v-else>
