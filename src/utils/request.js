...
const service = axios.create({
    baseURL: process.env.VUE_APP_MOCK_API_URL, // api 的 base_url
...
service.interceptors.response.use(
    response => {
        ...
        if ((response.status === 200 || response.status == true) && response.config.responseType === 'blob') { // 文件类型特殊处理
        ...
        } else if (response.status !== true && response.status !== 200) {
