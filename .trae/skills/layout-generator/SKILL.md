...
3.  **资源名称强制绑定**：
    - **关键原则**：资源文件名是人为手动命名的，具有**绝对权威性**，优先级**高于**UI视觉识别。
    - **强制映射**：
      - 发现 `rightTool-bg.png` → **必须**启用 `:rightTools="true"` 插槽
      - 发现 `leftTool-bg.png` → **必须**启用 `:leftTools="true"` 插槽
      - 发现 `headerTool-bg.png` → **必须**启用 `:headerTool="true"` 插槽
      - 发现 `footerTool-bg.png` → **必须**启用 `:footerTool="true"` 插槽
    - **严禁跨位置复用**：`rightTool` 资源**不得**用于底部/顶部/左侧工具栏。
4.  **推断布局能力**：
    - 根据已有资源推断当前项目支持的布局元素。
    - 为后续 UI 分析建立"资源基线"，明确哪些元素可以直接应用。
...
- [ ] **编译检查**：
  ...
  - 导入导出语句是否正确无误？
  - **严禁重复导入**：检查是否有重复的 import 语句（如同一个组件被导入两次）。
...
14. **导入语句唯一性**：
    - **严禁**在同一文件中重复导入同一个模块。
    - 使用批量替换工具时，必须确保替换范围不重叠，避免重复添加 import 语句。
...
17. **资源名称绝对优先原则**（🔴 最高优先级）：
    ...
    - **核心规则**：资源文件名是人为手动定义的，具有**绝对权威性**，优先级**绝对高于**UI视觉识别。
    - **强制映射逻辑**：
      1. Step 0 扫描资源 → 发现 `rightTool-bg.png`
      2. **立即确定**：必须使用 `:rightTools="true"` 插槽（右侧工具栏）
      3. Step 1 UI分析 → 即使视觉上看起来像底部工具栏，也**必须**理解为右侧工具栏的体现
      4. **严禁**因UI视觉判断而改用 `:footerTool="true"` 或其他插槽
    - **典型案例**：
      - ✅ 发现 `rightTool-bg.png` → 必须使用 `:rightTools="true"`，资源名称即定义
      - ✅ 发现 `leftTool-bg.png` → 必须使用 `:leftTools="true"`
      - ❌ 发现 `rightTool-bg.png`，但UI图底部有按钮 → 错误地使用 `:footerTool="true"` **绝对禁止**
    - **自检问题**：
      - "我是否严格按照资源名称选择了插槽？"
      - "我是否因为UI视觉判断而改变了资源的应用位置？"

18. **资源补全原则**（辅助资源优先原则）：
    
    - **执行逻辑**：
      1. Step 0 扫描资源 → 记录所有可用文件（`leftTool-bg.png`, `header-date.png` 等）
      2. Step 1 视觉识别 → 分析 UI 图布局
      3. **资源补全** → 将资源清单中存在但 UI 未识别的元素补充到实现清单
      4. 最终实现 = 资源强制映射 + UI 识别结果 + 资源补全项
    - **典型案例**：
      - ✅ 发现 `header-date.png` → 即使 UI 图模糊，也在 Header 中添加日期组件
      - ❌ UI 图看不清侧边工具，资源存在但未启用 → **错误**
